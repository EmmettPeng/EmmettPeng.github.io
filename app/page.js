...
export default function Home() {
  ...
  return <main className="page">
    <div className="identity"><div><h1>{home.name} <span>{home.chineseName} <strong className="name-pinyin">{home.pinyinName}</strong></span></h1><p className="muted">{home.tagline}</p><p className="link-row">{home.links.map(link => <a key={link.label} href={link.href}>{link.label}</a>)}</p></div><Image className={styles.avatar} src={home.avatar} alt={home.name} width={168} height={210} priority /></div>
